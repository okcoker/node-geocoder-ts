...
import TeleportProvider from 'src/provider/teleport/TeleportProvider';
...
describe('TeleportProvider', () => {
  ...
  describe('#constructor', () => {
    test('an http adapter must be set', () => {
      expect(() => {
        new TeleportProvider('' as unknown as HTTPAdapter);
      }).toThrow('TeleportProvider need an httpAdapter');
    });

    test('Should be an instance of TeleportProvider', () => {
      const adapter = new TeleportProvider(mockedHttpAdapter);

      expect(adapter).toBeInstanceOf(TeleportProvider);
    });
  });
  ...
  describe('#geocode', () => {
    ...
    test('Should not accept IPv6', async () => {
      ...
      ).rejects.toThrow('TeleportProvider does not support geocoding IPv6');
    });
