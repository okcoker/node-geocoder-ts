import OpenDataFranceProvider from 'src/provider/opendatafrance/OpenDataFranceProvider';
...
describe('OpenDataFranceProvider', () => {
  afterEach(() => {
    jest.resetAllMocks();
  });
  describe('#constructor', () => {
    test('an http adapter must be set', () => {
      expect(() => {
        new OpenDataFranceProvider('' as unknown as HTTPAdapter, {});
      }).toThrow('OpenDataFranceProvider need an httpAdapter');
    });
    test('Should be an instance of OpenDataFranceProvider', () => {
      const openDataFranceProvider = new OpenDataFranceProvider(
        mockedHttpAdapter,
        {}
      );

      expect(openDataFranceProvider).toBeInstanceOf(OpenDataFranceProvider);
    });
  });
  describe('#geocode', () => {
    ...
    test('Should not accept IPv6', async () => {
      ...
      expect(
        openDataFranceProvider.geocode(
