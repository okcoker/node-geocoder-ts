...
import LocationIQProvider from 'src/provider/locationiq/LocationIQProvider';
...
describe('LocationIQProvider', () => {
  ...
  describe('#constructor', () => {
    test('an http adapter must be set', () => {
      expect(() => {
        new LocationIQProvider('' as unknown as HTTPAdapter);
      }).toThrow('LocationIQProvider need an httpAdapter');
    });

    test('must have an api key as second argument', () => {
      expect(() => {
        new LocationIQProvider(mockedHttpAdapter);
      }).toThrow('LocationIQProvider needs an apiKey');
    });

    test('Should be an instance of LocationIQProvider', () => {
      const adapter = new LocationIQProvider(mockedHttpAdapter, {
        apiKey: 'API_KEY'
      });
      expect(adapter).toBeInstanceOf(LocationIQProvider);
    });
  });
