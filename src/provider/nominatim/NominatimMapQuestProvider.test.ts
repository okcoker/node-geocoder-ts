...
import NominatimMapQuestProvider from 'src/provider/nominatim/NominatimMapQuestProvider';
...
describe('NominatimMapQuestProvider', () => {
  ...
  describe('#constructor', () => {
    test('an http adapter must be set', () => {
      expect(() => {
        new NominatimMapQuestProvider('' as unknown as HTTPAdapter, {
          apiKey: ''
        });
      }).toThrow('NominatimMapQuestProvider need an httpAdapter');
    });

    test('an apiKey must be set', () => {
      expect(() => {
        new NominatimMapQuestProvider(mockedHttpAdapter, { apiKey: '' });
      }).toThrow('NominatimMapQuestProvider needs an apiKey');
    });

    test('Should be an instance of NominatimMapQuestProvider', () => {
      const adapter = new NominatimMapQuestProvider(mockedHttpAdapter, {
        apiKey: 'API_KEY'
      });

      expect(adapter).toBeInstanceOf(NominatimMapQuestProvider);
    });
  });
  ...
  describe('#geocode', () => {
    ...
    test('Should not accept IPv6', async () => {
      ...
      await expect(
        ...
        new ValueError(
          'NominatimMapQuestProvider does not support geocoding IPv6'
        )
      );
    });
