...
describe('DataScienceToolkitProvider', () => {
  afterEach(() => {
    jest.resetAllMocks();
  });
  ...
  describe('#constructor', () => {
    test('an http adapter must be set', () => {
      expect(() => {
        new DataScienceToolkitProvider('' as unknown as HTTPAdapter, {});
      }).toThrow('DataScienceToolkitProvider need an httpAdapter');
    });

    test('Should be an instance of DataScienceToolkitProvider', () => {
      const geocoder = new DataScienceToolkitProvider(mockedHttpAdapter, {});

      expect(geocoder).toBeInstanceOf(DataScienceToolkitProvider);
    });
  });
